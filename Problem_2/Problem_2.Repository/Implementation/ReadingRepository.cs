    ...
    public class ReadingRepository : IReadingRepository
    {
       private readonly AppDbContext context;
        ...
        public object GenerateTimeSeriesReport(Int16? buildingId, byte? objectId, byte? datafildId, DateTime startDate, DateTime endDate)
        {
            ...
            else if(differece.Days == 2)
            {
                topValue = "top 28800";
            }
                
            else if (differece.Days == 29 )
            {
                topValue = "top 432000";
            }
            else if (differece.Days == 60)
            {
                topValue = "top 864000";
            }
            else if (differece.Days == 89 )
            {
                topValue = "top 1296000";
            }



            var query= "select "+ topValue + "  * from Reading where BuildingId=" + buildingId + " and ObjectId=" + objectId + " and DataFieldId=" + datafildId + " and convert(date,TimeStamp) between '" + startDate + "' and '" + endDate + "'";
